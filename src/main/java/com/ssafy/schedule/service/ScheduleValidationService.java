...
public class ScheduleValidationService {
  ...
  public void validate(Planner planner, List<ScheduleDto> schedules) {
    ...
    if (schedules.size() == 1) { // 단일 스케줄인 경우 날짜만 확인
      LocalDate thisDate = schedules.get(0).date();
      if (planner.getStartDay().isBefore(thisDate)
          || planner.getEndDay().isAfter(thisDate)) {
