...
public class PlannerServiceImpl implements PlannerService {
	...
	public void createPlanner(PlannerCreateRequest request, CustomUserDetails loginUser) {
		...
		if (request.startDay().isAfter(request.endDay())) {
			throw new RuntimeException("[ERROR] 시작일이 종료일보다 늦을 수 없습니다.");
		}

		Planner planner = Planner.builder().name(request.name()).startDay(request.startDay()).endDay(request.endDay())
				.member(loginUser.getMember()).location(request.location()).build();
	...
	public List<Planner> getPlannersByMember(CustomUserDetails loginUser) {
		List<Planner> planners = plannerMapper.getPlannersByMember(loginUser.getMember().getId());
	...
	public Planner getPlannerById(Long plannerId, CustomUserDetails loginUser) {
		...
		if (planner.getMember().getId() != loginUser.getMember().getId()) {
	...
	@Override
	public void updatePlanner(Long plannerId, PlannerUpdateRequest request, CustomUserDetails loginUser) {
		...
		if (targetPlanner.getMember().getId() != loginUser.getMember().getId()) {
			throw new RuntimeException("[ERROR] 사용자가 일치하지 않습니다.");
		}
		...
		planMapper.deletePlansByPlannerAndDate(plannerId, request.startDay(), request.endDay());

		Planner planner = Planner.builder().id(request.id()).name(request.name()).startDay(request.startDay())
				.endDay(request.endDay()).member(loginUser.getMember()).location(request.location()).build();
	...
	public void deletePlanner(Long plannerId, CustomUserDetails loginUser) {
		planMapper.deletePlansByPlanner(plannerId);
	...
	@Override
	public List<Plan> getPlansByPlanner(Long plannerId, CustomUserDetails loginUser) {
		...
		List<Plan> plans = planMapper.getPlansByPlanner(plannerId);
	...
	@Override
	public List<Plan> getPlansByShared(Long plannerId) {
		List<Plan> plans = planMapper.getPlansByPlanner(plannerId);
		if (plans == null) {
			throw new RuntimeException("[ERROR] 플래너가 존재하지 않습니다.");
		}
	...
	@Override
	public void updatePlansForPlanner(Long plannerId, List<PlanUpdateRequest> requests, CustomUserDetails loginUser) {
		...
		for (PlanUpdateRequest request : requests) {
			...
			if (request.date().isEqual(preDate)) {
				if (request.startTime().equals(preStartTime)) {
					throw new RuntimeException("[ERROR] 같은 날짜에 시작 시간이 중복된 일정이 존재합니다.");
				}

				if (request.startTime().isBefore(preEndTime)) {
		...
		List<Plan> plans = requests.stream().map(request -> toEntity(request, planner)).collect(Collectors.toList());
		planMapper.updatePlansBatch(plans);
	}

	public void deletePlansForPlanner(Long plannerId, List<Long> planIdList, CustomUserDetails loginUser) {
		Planner planner = plannerMapper.getPlannerById(plannerId);
		...
		planMapper.deletePlansBatch(planIdList);
	}

	public Plan toEntity(PlanUpdateRequest request, Planner planner) {
		return Plan.builder().id(request.id()).date(request.date()).startTime(request.startTime())
				.endTime(request.endTime()).placeName(request.placeName()).address(request.address()).lat(request.lat())
				.lon(request.lon()).content(request.content()).planner(planner).placeUrl(request.placeUrl()).build();
	}
}
